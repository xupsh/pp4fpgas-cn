...
A common digital signal processing task that performs a convolution on the input signal with a fixed signal that is defined by its coeffcients. The FIR is often performed in hardware and can be efficiently implemented.
## FPGA
A field-programmable gate array (FPGA) is an integrated circuit that can be customized or programmed after it is manufactured ("in the field").
## HLS
